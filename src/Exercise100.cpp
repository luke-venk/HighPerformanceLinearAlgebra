...
public:
    ...
    Matrix BlockedMatMult(Matrix& other) {
        ...
        // Begin recursive algorithm multiplying A*B = C, with the initial dimension just being the dimension of A
        RecursiveMatMult(other, C, M);
        return C;
    }
...
int main() {
    ...
    cout << "FLOPS = " << FLOPS << ", GFLOPS = " << FLOPS/(1.e9) << endl;
    ...
    if (C.equalMatrices(D)) {
        cout << "Matrix product results match!" << endl;
    } else {
